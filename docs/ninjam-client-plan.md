...
- **Standalone harness** (no Rack/GUI) per phase — connect to a real public server for end-to-end,
  and to a **local private server** (build the server from `~/work/github/ninjam`) for offline,
  no-etiquette-risk testing.
