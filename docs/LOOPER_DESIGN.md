...
| M0 | `JamClock` extraction in Ninjam (integer counting, `gridGeneration`, `sessionFrame`), `JamClock.hpp`, expander chain walk; Looper scaffold (slug, 44 HP placeholder, `plugin.json`) with one light driven by `downbeat` | the sync path, before any looper logic |
