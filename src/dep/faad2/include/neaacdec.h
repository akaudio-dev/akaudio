...
#ifdef _WIN32
  #pragma pack(push, 8)
  /* akaudio local patch: FAAD2 is compiled straight INTO plugin.dll, never shipped
     as its own DLL. Leaving the API `__declspec(dllexport)` flips MinGW out of its
     export-everything default (it exports only decorated symbols), which drops
     Rack's `init` from plugin.dll and makes Rack refuse to load us ("Failed to read
     init() symbol"). Export nothing from here so `init` auto-exports as normal. */
  #define NEAACDECAPI
